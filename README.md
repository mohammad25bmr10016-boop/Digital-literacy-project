Project Overview :
This repository serves as my complete Digital Literacy Portfolio, created for my role as a Student Digital Ambassador. The goal of this project is to showcase essential digital skills required to navigate the online world safely and professionally. It includes visual resources, professional profile setups, coding practice evidence, communication guides, and cybercrime awareness materials.



My Profile:

Name:MOHAMMED SIDHAN  
Registration Number:25BMR10016  
Branch:MECH(AI and Robotics)



Module Summaries :

Module 1: Digital Literacy Awareness InfographicFor this module, I created a one-page infographic using Canva to help peers understand core digital concepts. The visual guide specifically covers what digital literacy is, highlights useful digital tools for computer science students, and outlines key professional email etiquette rules.

Module 2: Build Your Student Digital PortfolioA professional digital presence is essential for connecting with the academic and tech communities. I established my foundational portfolio by setting up and updating professional profiles on GitHub, LinkedIn, and Kaggle. These platforms will host my code repositories, track my academic milestones, and allow me to participate in data science competitions.

Module 3: Explore Coding \& Collaboration PlatformsTo build familiarity with online practice and collaboration tools , I completed a beginner Python coding challenge on HackerRank. Additionally, I designed a 5-question 'Digital Literacy Awareness Quiz' using Google Forms, which is linked to a Google Sheet to automatically track and organize peer responses.

Module 4: Draft a Professional Email \& Etiquette GuideClear digital communication is critical for academic and professional success. I drafted two formal emails—one requesting a deadline extension and another applying for an internship—adhering to strict professional formatting rules. I also compiled a Do's and Don'ts checklist for responsible social media use.

Module 5: Cybercrime Awareness Case Study \& Prevention GuideTo help peers recognize online threats , I researched UPI and Online Payment Fraud to write a detailed case study. Accompanying this is a 'Stay Safe Online' prevention checklist tailored for college students in India, highlighting actionable steps to protect financial data and outlining proper reporting channels like the National Cyber Crime Portal



Link of the Google Quiz form :
https://forms.gle/NMt3ZCSTZCiziKKt7

